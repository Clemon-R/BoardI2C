...
static void IRAM_ATTR _TFT_pushColorRep2(color_t *color, uint32_t len, uint8_t rep, uint8_t wait)
{
	if (len == 0) return;
	if (!(disp_spi->cfg.flags & LB_SPI_DEVICE_HALFDUPLEX)) return;
	...
	if ((len*24) <= 512) {
		...
		*/
		...
		buf_colors = ((len > (_width*5)) ? (_width*5) : len);
		buf_bytes = buf_colors * 3;
...
void IRAM_ATTR TFT_pushColorRep2(int x1, int y1, int x2, int y2, color_t *color, uint32_t len)
{
	...
	_TFT_pushColorRep2(color, len, 1, 1);
